...
function checkMobile() {
    ...
    // If more than 10 digits are entered, trim the input value to 10 digits
    if (mobile.length > 10) {
        document.getElementById('mobileNumber').value = mobile.slice(0, 10);
        mobile = document.getElementById('mobileNumber').value;  // update mobile variable after trimming
    }
    
    // Show 'Send OTP' button when exactly 10 digits are entered
    if (mobile.length === 10) {
...
function sendOTP() {
    ...
    // Only proceed if the mobile number is exactly 10 digits
    if (mobile.length === 10) {
        ...
        // Call the function to send OTP via WhatsApp
        sendOTPWhatsApp(mobile);
    }
}

function sendOTPWhatsApp(mobile) {
    // Logic to send OTP via WhatsApp
    alert("An OTP has been sent to " + mobile + " via WhatsApp.");
}
